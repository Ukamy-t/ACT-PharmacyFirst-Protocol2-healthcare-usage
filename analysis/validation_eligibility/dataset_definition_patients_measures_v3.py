...
from analysis.pf_variable_library import check_code_in_time_window, check_recurrent_status, get_recurrent_anchor_date
...
# v2: anchor is the day before the monthly interval start
# impetigo_exclusion_anchor_date = start_date
# v3: If PF impetigo exists in the month, anchor is the day before first PF impetigo consultation.
# Otherwise, anchor is the day before the monthly interval start.
pf_impetigo_events = selected_pf_id_events.where(selected_pf_id_events.snomedct_code.is_in(codelists.impetigo_code))
# impetigo_exclusion_anchor_date = start_date
impetigo_exclusion_anchor_date = get_recurrent_anchor_date(start_date,pf_impetigo_events,)

# bullous_impetigo during the specific month
# bullous impetigo in the month before the impetigo eligibility anchor date.
...
bullous_impetigo_this_month = check_code_in_time_window(impetigo_exclusion_anchor_date-months(1),impetigo_exclusion_anchor_date,clinical_events,codelists.gp_snomed_codelist_bullous_impetigo)
...
# v2: Recurrent impetigo status at the start of the monthly interval.
# v3: Recurrent impetigo is assessed before PF impetigo use, or at the start of the month for patients without PF impetigo use.
recurrent_impetigo_this_year = check_recurrent_status(impetigo_exclusion_anchor_date, clinical_events, codelists.gp_snomed_codelist_impetigo,
                                                      lookback_months=12, gap_weeks=4, min_episodes=2)
...
# v3: firts PF UTI date or month start
pf_uti_events = selected_pf_id_events.where(selected_pf_id_events.snomedct_code.is_in(codelists.uti_code))
uti_exclusion_anchor_date = get_recurrent_anchor_date(start_date,pf_uti_events,)
...
catheter_status = check_code_in_time_window(uti_exclusion_anchor_date - months(12),uti_exclusion_anchor_date,clinical_events,codelists.gp_snomed_codelist_urinary_catheter)
...
# )
# v2: Recurrent UTI status at the start of the monthly interval - captures whether the patient already met the recurrent UTI exclusion before the current month began.
# v3: Recurrent UTI is assessed before PF impetigo use, or at the start of the month for patients without PF impetigo use.
recurrent_uti_6m = check_recurrent_status(
    uti_exclusion_anchor_date, clinical_events, codelists.gp_snomed_codelist_uti,
    lookback_months=6, gap_weeks=4,min_episodes=2
)
recurrent_uti_12m = check_recurrent_status(
    uti_exclusion_anchor_date, clinical_events, codelists.gp_snomed_codelist_uti,
